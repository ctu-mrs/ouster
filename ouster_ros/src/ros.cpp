...
sensor_msgs::Imu packet_to_imu_msg(const PacketMsg& p, const std::string& frame, const sensor::packet_format& pf) {
...
void scan_to_cloud(const ouster::XYZLut& xyz_lut, ouster::LidarScan::ts_t scan_ts, const ouster::LidarScan& ls, ouster_ros::Cloud& cloud) {
  ...
  for (auto u = 0; u < ls.h; u++) {
    for (auto v = 0; v < ls.w; v++) {
      ...
      cloud(v, u)    = ouster_ros::Point{{{static_cast<float>(xyz(0)), static_cast<float>(xyz(1)), static_cast<float>(xyz(2)), 1.0f}},
...
void scan_to_cloud(const ouster::XYZLut& xyz_lut, ouster::LidarScan::ts_t scan_ts, const ouster::LidarScan& ls, ouster_ros::Cloud& cloud,
                   std::shared_ptr<tf::TransformListener> listener, const std::string& fixed_frame, const std::string& sensor_frame, const double& waitForTransform) {
  ...
  if (waitForTransform > 0.0) {
    ...
    ROS_INFO_STREAM("waitForTransform " << waitForTransform);
    ROS_INFO_STREAM("sensor_frame " << sensor_frame);
    ROS_INFO_STREAM("start_stamp " << start_stamp);
    ROS_INFO_STREAM("sensor_frame " << sensor_frame);
    ROS_INFO_STREAM("errorMsg " << errorMsg);
    ROS_INFO_STREAM("sensor_frame " << fixed_frame);
    ROS_INFO_STREAM("pt_stamp " << pt_stamp);

    if (!listener->waitForTransform(sensor_frame, start_stamp, sensor_frame, pt_stamp, fixed_frame, ros::Duration(waitForTransform), ros::Duration(0.0001),
                                   &errorMsg)) {

      ROS_INFO("pes4");
      ROS_WARN(
          "Could not estimate motion of %s accordingly to fixed "
          "frame %s, returning possible skewed cloud! (%s)",
          sensor_frame.c_str(), fixed_frame.c_str(), errorMsg.c_str());
      // Do original copy
  ...
  for (auto v = 0; v < ls.w; v++) {
    ...
    try {
      listener->lookupTransform(sensor_frame, start_stamp, sensor_frame, pt_stamp, fixed_frame, transform);
      transformValid = true;
    }
    ...
    for (auto u = 0; u < ls.h; u++) {
      ...
      auto pt = tf::Point(static_cast<float>(xyz(0)), static_cast<float>(xyz(1)), static_cast<float>(xyz(2)));
      if (transformValid) {
        pt = transform * pt;
      }

      cloud(v, u) = ouster_ros::Point{{{(float)pt.x(), (float)pt.y(), (float)pt.z(), 1.0f}},
                                      static_cast<float>(pix(ouster::LidarScan::INTENSITY)),
                                      static_cast<uint32_t>(ts),
                                      static_cast<uint16_t>(pix(ouster::LidarScan::REFLECTIVITY)),
                                      static_cast<uint8_t>(u),
  ...
  if (transformsNotValid != 0) {
    ROS_WARN(
        ...
        sensor_frame.c_str(), fixed_frame.c_str(), transformsNotValid * cloud.height, (int)cloud.size());
  }
}

sensor_msgs::PointCloud2 cloud_to_cloud_msg(const Cloud& cloud, ns timestamp, const std::string& frame) {
...
geometry_msgs::TransformStamped transform_to_tf_msg(const ouster::mat4d& mat, const std::string& frame, const std::string& child_frame) {
  Eigen::Affine3d aff;
